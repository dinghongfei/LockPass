...
>
> 快速上手见 [README.zh-CN.md](README.zh-CN.md)（[English README](README.md)）。
...
| 预置用户 | `users.local.json` / `users.json` 配置，不支持注册 |
...
flowchart TB
    ...
    subgraph data [配置与数据]
        Users["config/users.json 或 users.local.json"]
        Env["环境变量 .env"]
...
# 4. 配置用户（可选，默认 admin/admin123；生产建议 users.local.json）
npm run hash-password -- your-password
# 将输出的 hash 写入 config/users.local.json（或 users.json）
...
lock-pass/
├── config/
│   ├── users.json                 # 仓库默认用户（admin / admin123）
│   └── users.local.json           # 生产覆盖（gitignore；服务器优先使用）
├── data/                          # 运行时数据目录（gitignore，勿提交）
│   ├── vault.db                   # SQLite 模式
│   └── vault.json                 # Text 模式
├── scripts/
│   ├── hash-password.ts           # bcrypt 密码 hash 生成工具
│   └── service.sh                 # 生产启停：start / stop / restart / status
├── src/
...
flowchart TB
    ...
    subgraph persistence [配置与持久化]
        direction TB
        UsersFile["users.json / users.local.json"]
        EnvConfig[".env 环境变量"]
...
1. 环境变量 `USERS_FILE`（若设置）
2. `config/users.local.json`（若存在；**已被 gitignore**，生产机改密用此文件）
...
npm run hash-password -- your-password
# 更新 config/users.local.json 的 passwordHash，然后重启
```
...
| `USERS_FILE` | 否 | — | 用户配置路径（覆盖默认；未设置时按 users.local.json → users.json） |
...
将输出的 hash 追加到**当前生效**的用户文件（服务器上优先 `users.local.json`），重启服务。
...
编辑**当前生效**的用户文件（服务器上优先 `config/users.local.json`），用 `npm run hash-password -- 新密码` 生成新 hash 替换，重启服务。
